    ...
    func StackPlayerView(size: CGSize) -> some View {
        ...
        ZStack {
            ForEach(stackAlbums.reversed()) { album in
                ...
                // MARK: - 3D Rotation
                    .rotation3DEffect(.init(degrees: expandCards && currentIndex != index ? -10 : 0), axis: (x: 1, y: 0, z: 0), anchor: UnitPoint.center, anchorZ: 1, perspective: 1)
                // Rotation Current Card for 360 degree
                // You can customize this if you want rotation animation when closing
                    .rotation3DEffect(.init(degrees: showDetail && currentIndex == index  && rotateCards ? 360 : 0), axis: (x: 1, y: 0, z: 0), anchor: UnitPoint.center, anchorZ: 1, perspective: 1)
